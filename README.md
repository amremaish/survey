...
## Troubleshooting
- Missing `redis`/`db` containers: `docker compose ps` and `docker compose logs redis db`
- Email issues: verify SMTP values in `.env`; test via Django shell
- Swagger not loading: confirm `drf-spectacular` installed and `/api/schema/` reachable
